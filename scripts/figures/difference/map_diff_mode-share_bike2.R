...
message(sprintf("Difference: %s -> %s", scenario0, scenario1))

results0 <- readr::read_rds(here::here("results", sprintf("zones_%s.rds", scenario0))) %>%
  dplyr::select(zone, mode_share_bike) %>%
  dplyr::rename(mode_share_bike0 = mode_share_bike)

results1 <- readr::read_rds(here::here("results", sprintf("zones_%s.rds", scenario1))) %>%
  sf::st_drop_geometry() %>%
  dplyr::select(zone, mode_share_bike) %>%
  dplyr::rename(mode_share_bike1 = mode_share_bike)

results <- results0 %>%
  dplyr::left_join(results1, by = "zone") %>%
  dplyr::mutate(diff_mode_share_bike = mode_share_bike1 - mode_share_bike0)
...
ggplot() +
  geom_sf(mapping = aes(fill = diff_mode_share_bike),
  ...
  coord_sf_mal() +
  annotate_map(
    title = "Muutos polkupyörällä tehtyjen kiertomatkojen osuuksissa alueelta alkavista kiertomatkoista",
...
ggsave_map(here::here("figures", sprintf("map_diff_mode-share_bike_%s-%s.png", scenario0, scenario1)))
