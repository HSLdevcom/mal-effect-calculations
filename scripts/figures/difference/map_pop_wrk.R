# -*- coding: utf-8-unix -*-
library(here)
library(tidyverse)
library(sf)
source(here::here("scripts", "basemap", "functions_map.R"), encoding = "utf-8")


# Data --------------------------------------------------------------------

scenario0 <- "2018"
scenario1 <- "2040_ve0"

results0 <- readr::read_rds(here::here("results", sprintf("zones_%s.rds", scenario0))) %>%
  dplyr::select(zone, total_pop, total_wrk, land_area) %>%
  dplyr::rename(wrk0 = total_wrk,
                pop0 = total_pop)

results1 <- readr::read_rds(here::here("results", sprintf("zones_%s.rds", scenario1))) %>%
  sf::st_drop_geometry() %>%
  dplyr::select(zone, total_pop, total_wrk) %>%
  dplyr::rename(wrk1 = total_wrk,
                pop1 = total_pop)

results <- results0 %>%
  dplyr::left_join(results1, by = "zone") %>%
  dplyr::mutate(
    # One scenario
    pop_density0 = pop0 / land_area,
    pop_density1 = pop1 / land_area,
    wrk_density0 = wrk0 / land_area,
    wrk_density1 = wrk1 / land_area,
    # Population differences
    pop_diff = pop1 - pop0,
    pop_diff_rel = pop_diff / pop0,
    pop_density_diff = pop_density1 - pop_density0,
    # Workplace dfferences
    wrk_diff = wrk1 - wrk0,
    wrk_diff_rel = wrk_diff / wrk0,
    wrk_density_diff = wrk_density1 - wrk_density0,
  ) %>%
  dplyr::mutate(
    pop_diff_rel = dplyr::if_else(is.nan(pop_diff_rel), 0, pop_diff_rel),
    wrk_diff_rel = dplyr::if_else(is.nan(wrk_diff_rel), 0, wrk_diff_rel)
  )

quantile_ <- function(x, probs = c(0, 0.01, 0.02, 0.05, 0.95, 0.98, 0.99, 1), ...) {
  quantile(x = x, probs = probs, ...)
}

quantile_(results$pop_density0)
quantile_(results$pop_density1)
quantile_(results$pop_density_diff)

quantile_(results$wrk_density0)
quantile_(results$wrk_density1)
quantile_(results$wrk_density_diff)

# Plot pop_density_diff ---------------------------------------------------

ggplot() +
  geom_sf(mapping = aes(fill = pop_density_diff),
          data = results, color = NA) +
  # geom_sf(mapping = aes(color = ''),
  #         data = dplyr::filter(results, pop_density_diff < 0.0001), fill = "#ffffff") +
  scale_fill_distiller(
    palette = "Spectral",
    name = "asukasta per km2",
    labels = scales::label_number(accuracy = 1),
    direction = -1,
    limits = c(-5000, 5000),
    oob = scales::squish
  ) +
  scale_color_manual(
    values = NA
  ) +
  # guides(fill = guide_colorbar(order = 1),
  #        color = guide_legend("Ei muutosta\ntai muutos on\nnegatiivinen",
  #                             order = 100,
  #                             override.aes = list(color = "#333333"),
  #                             keywidth = unit(0.6, "cm"),
  #                             keyheight = unit(0.6, "cm"))) +
  geom_basemap() +
  geom_sf(data = results, fill = NA, color = "#333333", size = 0.1) +
  coord_sf_mal() +
  annotate_map(
    title = "Väestötiheyden absoluuttinen muutos",
    subtitle = sprintf("%d %s \U2192 %d %s",
                       scenarios$year[scenarios$scenario == scenario0],
                       scenarios$name[scenarios$scenario == scenario0],
                       scenarios$year[scenarios$scenario == scenario1],
                       scenarios$name[scenarios$scenario == scenario1])
  ) +
  theme_mal_map()

ggsave_map(here::here("figures", sprintf("map_diff_pop_density_%s_%s.png", scenario0, scenario1)))


# Plot wrk_density_diff ---------------------------------------------------

ggplot() +
  geom_sf(mapping = aes(fill = wrk_density_diff),
          data = results, color = NA) +
  # geom_sf(mapping = aes(color = ''),
  #         data = dplyr::filter(results, wrk_density_diff < 0.0001), fill = "#ffffff") +
  scale_fill_distiller(
    palette = "Spectral",
    name = "työpaikkoja per km2",
    labels = scales::label_number(accuracy = 1),
    direction = -1,
    limits = c(-3000, 3000),
    oob = scales::squish
  ) +
  scale_color_manual(
    values = NA
  ) +
  # guides(fill = guide_colorbar(order = 1),
  #        color = guide_legend("Ei muutosta\ntai muutos on\nnegatiivinen",
  #                             order = 100,
  #                             override.aes = list(color = "#333333"),
  #                             keywidth = unit(0.6, "cm"),
  #                             keyheight = unit(0.6, "cm"))) +
  geom_basemap() +
  geom_sf(data = results, fill = NA, color = "#333333", size = 0.1) +
  coord_sf_mal() +
  annotate_map(
    title = "Työpaikkatiheyden absoluuttinen muutos",
    subtitle = sprintf("%d %s \U2192 %d %s",
                       scenarios$year[scenarios$scenario == scenario0],
                       scenarios$name[scenarios$scenario == scenario0],
                       scenarios$year[scenarios$scenario == scenario1],
                       scenarios$name[scenarios$scenario == scenario1])
  ) +
  theme_mal_map()

ggsave_map(here::here("figures", sprintf("map_diff_wrk_density_%s_%s.png", scenario0, scenario1)))

# Plot pop_density_2018 ------------------------------------------

ggplot() +
  geom_sf(mapping = aes(fill = pop_density0),
          data = results, color = NA) +
  scale_fill_distiller(
    palette = "YlGn",
    name = "asukasta per km2",
    labels = scales::label_number(accuracy = 1),
    direction = 1,
    limits = c(0, 10000),
    oob = scales::squish
  ) +
  scale_color_manual(
    values = NA
  ) +
  geom_basemap() +
  geom_sf(data = results, fill = NA, color = "#333333", size = 0.1) +
  coord_sf_mal() +
  annotate_map(
    title = "Väestötiheys",
    subtitle = sprintf("%d %s", scenarios$year[scenarios$scenario == scenario0],
                       scenarios$name[scenarios$scenario == scenario0])
  ) +
  theme_mal_map()

ggsave_map(here::here("figures", sprintf("map_pop_density_%s.png", scenario0)))


# Plot pop_density_2040_ve0 ------------------------------------------

ggplot() +
  geom_sf(mapping = aes(fill = pop_density1),
          data = results, color = NA) +
  scale_fill_distiller(
    palette = "YlGn",
    name = "asukasta per km2",
    labels = scales::label_number(accuracy = 1),
    direction = 1,
    limits = c(0, 10000),
    oob = scales::squish
  ) +
  scale_color_manual(
    values = NA
  ) +
  geom_basemap() +
  geom_sf(data = results, fill = NA, color = "#333333", size = 0.1) +
  coord_sf_mal() +
  annotate_map(
    title = "Väestötiheys",
    subtitle = sprintf("%d %s", scenarios$year[scenarios$scenario == scenario1],
                       scenarios$name[scenarios$scenario == scenario1])
  ) +
  theme_mal_map()

ggsave_map(here::here("figures", sprintf("map_pop_density_%s.png", scenario1)))


# Plot wrk_density_2018 ------------------------------------------

ggplot() +
  geom_sf(mapping = aes(fill = wrk_density0),
          data = results, color = NA) +
  scale_fill_distiller(
    palette = "OrRd",
    name = "työpaikkaa per km2",
    labels = scales::label_number(accuracy = 1),
    direction = 1,
    limits = c(0, 10000),
    oob = scales::squish
  ) +
  scale_color_manual(
    values = NA
  ) +
  geom_basemap() +
  geom_sf(data = results, fill = NA, color = "#333333", size = 0.1) +
  coord_sf_mal() +
  annotate_map(
    title = "Työpaikkatiheys",
    subtitle = sprintf("%d %s", scenarios$year[scenarios$scenario == scenario0],
                       scenarios$name[scenarios$scenario == scenario0])
  ) +
  theme_mal_map()

ggsave_map(here::here("figures", sprintf("map_wrk_density_%s.png", scenario0)))


# Plot wrk_density_2040_ve0 ------------------------------------------

ggplot() +
  geom_sf(mapping = aes(fill = wrk_density1),
          data = results, color = NA) +
  scale_fill_distiller(
    palette = "OrRd",
    name = "työpaikkaa per km2",
    labels = scales::label_number(accuracy = 1),
    direction = 1,
    limits = c(0, 10000),
    oob = scales::squish
  ) +
  scale_color_manual(
    values = NA
  ) +
  geom_basemap() +
  geom_sf(data = results, fill = NA, color = "#333333", size = 0.1) +
  coord_sf_mal() +
  annotate_map(
    title = "Työpaikkatiheys",
    subtitle = sprintf("%d %s", scenarios$year[scenarios$scenario == scenario1],
                       scenarios$name[scenarios$scenario == scenario1])
  ) +
  theme_mal_map()

ggsave_map(here::here("figures", sprintf("map_wrk_density_%s.png", scenario1)))

